...
function agaclarCiz() {
    //canvas'ın içine denk gelen ağaçları çiz
...
function ziplamayıYonet() {
    if (jd == 0) return;

    //dino max yüksekliğin 5/4 üneünün üstündeyse yavaş ilerlet
    if (jh < mjh * .8)
        jh += jd * js;//yerden yüksekliğini YAVAŞÇA arttır/azalt
    else {
        jh += jd * sjs;
    }
    ...
    //aşağı düşüyor ve yete çarptıysa
    if (jd < 0 && jh <= 0) {
        jd = 0;
    }
